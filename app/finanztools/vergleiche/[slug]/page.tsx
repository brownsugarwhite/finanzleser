...
export default async function VergleichDetailPage({ params }: Props) {
  ...
  if (FADEN_AKTIV) {
    return (
      <>
        {jsonLd}
        <KartenKapitel schluessel={`vergleich:${slug}`} titel={title} kicker="Anzeige · Vergleich mit Partnerlinks" beschreibung={beschreibung} krumen={[{ name: "Finanztools", href: "/finanztools" }, { name: "Vergleiche", href: "/finanztools/vergleiche" }]} url={pfad} eigenerKopf>
          ...
          <VergleichKoerper slug={slug} skin="faden" beschreibung={cleanDescription(excerpt || "")} />
