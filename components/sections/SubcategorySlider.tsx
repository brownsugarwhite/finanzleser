...
import SlideCategoryCard, { type CategorySlide, getCategoryCardWidth } from '@/components/ui/SlideCategoryCard';

const SLIDE_BASE_WIDTH = 350;
const GAP_FULL = 22;
const GAP_SHRUNK = 15;
...
const THRESHOLD_RESET_RATIO = 0.95;
...
export default function SubcategorySlider({ categories, parentSlug }: SubcategorySliderProps) {
  ...
  const updateProgress = useCallback(() => {
    ...
    for (let i = 1; i < newProgresses.length; i++) {
      const wPrev = getCategoryCardWidth(newProgresses[i - 1] ?? 0);
      const wCurr = getCategoryCardWidth(newProgresses[i] ?? 0);
  ...
  const getTranslateX = (index: number): number => {
    ...
    for (let i = 1; i <= index; i++) {
      const wPrev = getCategoryCardWidth(slideProgresses[i - 1] ?? 0);
      const wCurr = getCategoryCardWidth(slideProgresses[i] ?? 0);
      ...
      <div ref={emblaRef} style={{ overflow: 'hidden', cursor: 'grab', height: '390px', visibility: isReady ? 'visible' : 'hidden' }}>
        <div style={{
          ...
          {categories.map((cat, index) => {
            ...
            const cardW = getCategoryCardWidth(progress);
