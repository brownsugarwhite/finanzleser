...
export default function BookmarkNav() {
  ...
  const closeSearch = useCallback(() => {
    ...
    // Body shrinks
    tl.to(body, {
      width: bodyDefaultW.current,
      duration: 0.5,
      ease: "back.in(1.2)",
    });

    // Pill width + right + inner
    const bodyRect = body.getBoundingClientRect();
    const lupeRect = lupeRef.current?.getBoundingClientRect();
    tl.to(pill, {
      width: BTN_SIZE,
      right: lupeRect ? bodyRect.right - lupeRect.right : 0,
      duration: 0.5,
      ease: "back.in(1.2)",
    }, 0);
    tl.to(searchInnerRef.current, {
      x: innerStartX.current,
      duration: 0.5,
      ease: "back.in(1.2)",
    }, 0);
    ...
    if (burgerVisible.current) {
      tl.to(burgerWrapRef.current, { opacity: 1, duration: 0.25 }, 0.35);
    }
    tl.to(lupeRef.current, { opacity: 1, duration: 0.25 }, 0.35);

  }, []);

  /* ── Burger (X ↔ hamburger toggle) ── */

  const toggleBurgerX = useCallback(() => {
    const [top, mid, bot] = burgerLinesRef.current;
    ...
    if (burgerIsX.current) {
      ...
      const tl = gsap.timeline();
      tl.to([top, bot], { width: BURGER_LINE_W, duration: 0.25, ease: "power2.out" }, 0);
      tl.to(top, { y: 0, rotation: 0, duration: 0.3, ease: "power2.out" }, 0);
      tl.to(bot, { y: 0, rotation: 0, duration: 0.3, ease: "power2.out" }, 0);
      tl.to(mid, { opacity: 1, scaleX: 1, width: BURGER_LINE_W, duration: 0.3, ease: "power2.out" }, 0);
      window.dispatchEvent(new CustomEvent("burger-closed"));
    } else {
      // hamburger → X
      burgerIsX.current = true;
      const tl = gsap.timeline();
      tl.to(mid, { opacity: 0, scaleX: 0, duration: 0.2, ease: "power2.out" }, 0);
      tl.to(top, { y: BURGER_GAP + 2, rotation: 45, duration: 0.3, ease: "power2.out" }, 0);
      tl.to(bot, { y: -(BURGER_GAP + 2), rotation: -45, duration: 0.3, ease: "power2.out" }, 0);
      tl.to([top, bot], { width: BURGER_LINE_W * Math.sqrt(2), duration: 0.3, ease: "power2.out" }, 0);
  ...
  useEffect(() => {
    ...
    const onMegaClosed = () => {
      ...
      if (burgerIsX.current && burgerWrapRef.current && !searchOpen.current) {
        // Mega menu closed: collapse burger back to hamburger
        toggleBurgerX();
      }
    };
